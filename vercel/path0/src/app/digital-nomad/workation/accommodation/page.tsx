
import PageHeader from '@/components/shared/PageHeader';
import Section from '@/components/shared/Section';

export const metadata = {
  title: 'Premium Digital Nomad Accommodation | Apartments & Hotels',
  description: 'Find premium accommodation for digital nomads in the Costa del Sol. Explore apartments and hotels with great Wi-Fi and workspaces.',
};

export default function AccommodationPage() {
  return (
    <div>
      <PageHeader
        title="Premium Digital Nomad Accommodation"
        subtitle="Find comfortable and well-equipped accommodation for your remote work stay."
      />
      <Section>
        <p className="text-center text-lg">Placeholder content for premium digital nomad accommodation. This page will feature suitable apartments and aparthotels.</p>
      </Section>
    </div>
  );
}
