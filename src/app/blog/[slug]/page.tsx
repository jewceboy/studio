...
const articlesData: { [key: string]: Article } = {
  'best-tapas-malaga': {
    ...
    <div class="my-6 p-4 bg-accent-3-light-blue/10 border-l-4 border-accent-3-light-blue">
        <h4 class="font-montserrat font-semibold text-primary-dark">Insider Tip:</h4>
    ...
    <ul class="list-disc list-inside my-4 space-y-2">
        <li><a href="#" class="text-accent-1-red hover:underline">Malaga Old Town Tapas Tour</a></li>
        <li><a href="#" class="text-accent-1-red hover:underline">Boutique Hotels in Malaga Center</a></li>
    </ul>
    `,
  ...
  return (
    <div className="container mx-auto px-4">
      <article className="max-w-3xl mx-auto">
        ...
        <header className="mb-8 text-center">
          <div className="mb-4">
            {article.categories?.map(cat => (
              <Badge key={cat} variant="secondary" className="mr-2 bg-accent-3-light-blue/20 text-accent-3-light-blue font-montserrat">{cat}</Badge>
            ))}
          </div>
          <h1 className="font-anton text-4xl md:text-5xl font-bold text-primary-dark mb-4">{article.title}</h1>
        ...
        {/* Article Content */}
        <div
          className="prose prose-lg max-w-none text-foreground font-inter prose-headings:font-montserrat prose-headings:text-primary-dark prose-a:text-accent-1-red hover:prose-a:underline prose-img:rounded-lg prose-img:shadow-md"
        ...
        {article.tags && article.tags.length > 0 && (
            <div className="mt-8 py-4 border-t border-b">
                <span className="font-montserrat font-semibold mr-2 text-primary-dark">Tags:</span>
        ...
        {article.author && (
          <div className="mt-12 p-6 bg-card rounded-lg shadow-md flex items-start space-x-4">
            ...
            <div>
              ...
              <h4 className="font-montserrat text-xl font-semibold text-primary-dark mb-1">{article.author}</h4>
        ...
        <div className="mt-8 py-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="flex space-x-3">
                <Button variant="outline" size="icon" aria-label="Share on Facebook"><Share2 className="h-5 w-5 text-primary-dark" /></Button>
                <Button variant="outline" size="icon" aria-label="Share on Twitter"><Share2 className="h-5 w-5 text-primary-dark" /></Button>
                <Button variant="outline" size="icon" aria-label="Share on Pinterest"><Share2 className="h-5 w-5 text-primary-dark" /></Button>
                 <Button variant="outline" size="icon" aria-label="Print article" onClick={() => typeof window !== 'undefined' && window.print()}><Printer className="h-5 w-5 text-primary-dark" /></Button>
            </div>
            <Button variant="outline" className="border-accent-1-red text-accent-1-red hover:bg-accent-1-red hover:text-primary-light">
